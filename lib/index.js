...
function writeResumeMarker(sessionIds) {
	try {
		fs.mkdirSync(RUNTIME_DIR, { recursive: true });
		fs.writeFileSync(RESUME_MARKER_FILE, JSON.stringify({
			sessionIds,
...
function readResumeMarker() {
	try {
		const j = JSON.parse(fs.readFileSync(RESUME_MARKER_FILE, "utf8"));
...
function clearResumeMarker() {
	try {
		fs.unlinkSync(RESUME_MARKER_FILE);
	} catch {}
}
/**
...
`;
		...
		return {
			ok: true,
			action: "restart",
			note: "DeepSeek Harness 正在重启，约 10 秒后恢复。重启完成后 GET /api/dsh-restart-button/health 会返回 restarted:true 与 fromInstanceId 以确认新实例。"
		};
	} catch (e) {
		return {
			ok: false,
	...
	const interval = setInterval(() => {
		...
		for (const sessionId of [...pending]) {
			...
			try {
				...
				try {
					appendLog(LOG_FILE, `${(/* @__PURE__ */ new Date()).toISOString()} restart confirmation followup failed for ${sessionId}: ${String(error)}\n`);
				} catch {}
			}
