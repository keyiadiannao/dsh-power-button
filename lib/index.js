...
const HELPER_FILE = path.join(RUNTIME_DIR, "dsh-restart-helper.cjs");
const LOG_FILE = path.join(RUNTIME_DIR, "restart-helper.log");
/**
...
try {
	...
	fs.appendFileSync(LOG_FILE, `${(/* @__PURE__ */ new Date()).toISOString()} loaded execArgv=${JSON.stringify(process.execArgv)} argv=${JSON.stringify(process.argv)} cwd=${process.cwd()}\n`);
} catch {}
/**
...
(async () => {
  log('helper up, waiting for port ' + PORT + ' to free');
  let freed = false;
  for (let i = 0; i < 60; i++) {
    if (await portFree(PORT)) { freed = true; break; }
    await new Promise((r) => setTimeout(r, 500));
  }
  if (!freed) { log('port never freed - giving up'); return; }
  await new Promise((r) => setTimeout(r, 1200)); // settle: let the socket fully release
...
`;
		...
		return {
			ok: true,
			action: "restart",
			note: "DeepSeek Harness 正在重启，约 10 秒后恢复"
		};
	} catch (e) {
		return {
			ok: false,
...
function apply(ctx) {
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (sub === "/health" && req.method === "GET") return json(res, 200, {
					ok: true,
					v: createHash("sha256").update(String(Date.now())).digest("hex").slice(0, 8)
				});
				json(res, 404, {
					ok: false,
	...
	try {
		ctx.tools.register({
			...
			description: "重启整个 DeepSeek Harness 进程，用于重新加载插件与配置（profile 的 cordis 组合、settings 等）。由 dsh-restart-button 提供（独立实现）：派生一个 detach 的 helper，在旧进程退出并释放端口后以原命令行在原目录重新拉起，然后旧进程退出。触发后当前会话连接会短暂中断，网页随后自动重连到新进程。返回旧进程 pid、cwd、命令行与日志文件路径。",
			parameters: {
				type: "object",
				properties: { delayMs: {
					type: "number",
					description: "旧进程退出前等待的毫秒数（给当前结果留出回传时间），默认 2000。"
				} }
			},
			output: {
				schema: {},
			...
			async execute(args) {
				const a = args ?? {};
				return restartDsh(ctx, Number(a.delayMs) > 0 ? Math.floor(Number(a.delayMs)) : 2e3);
			}
		});
	} catch (error) {
		if (String(error).includes("already registered")) try {
			fs.appendFileSync(LOG_FILE, `${(/* @__PURE__ */ new Date()).toISOString()} restart_harness already registered by another plugin; skipping our tool\n`);
		} catch {}
		else throw error;
	}
}
