...
try {
	...
	appendLog(LOG_FILE, `${(/* @__PURE__ */ new Date()).toISOString()} loaded execArgv=${redactCommandLine(process.execArgv)} argv=${redactCommandLine(process.argv)} cwd=${process.cwd()}\n`);
} catch {}
/**
...
function restartConfirmationText(ctx) {
	try {
		const locale = ctx.settings?.get?.("locale");
		if ((typeof locale?.preference === "string" ? locale.preference.toLowerCase() : "").startsWith("en")) return "Restarted";
	} catch {}
	return "已重启";
}
				...
				try {
					session.append("assistant/message", {
						...
						message: createAssistantMessage({
							content: [{
								type: "text",
								text: restartConfirmationText(ctx)
							}],
							source: {
