...
function apply(ctx, config) {
	...
	ctx.effect(() => {
		...
		ctx.commands.register({
			name: "shutdown",
			description: en ? "Shut down DeepSeek Harness (stop process; restart manually). Requires a confirm argument." : "关机 DeepSeek Harness（停止进程，需手动重新启动）。需要 confirm 参数确认。",
			input: { hint: en ? "confirm — irreversible shutdown" : "confirm —— 不可逆关机" },
			recordInput: false,
			async handler(invocation) {
				const arg = invocation.rawInput.trim().toLowerCase();
				if (arg !== "confirm" && arg !== "yes") return {
					kind: "error",
					text: en ? "Shutdown is irreversible. Type `/shutdown confirm` to proceed." : "关机不可逆。请再输入 `/shutdown confirm` 确认关机。"
				};
