					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								...
								onMouseEnter: (e) => {
									e.currentTarget.style.background = "var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,0.14))";
								},
								onMouseLeave: (e) => {
		...
		function beginPower(next) {
			...
			if (next === "shutdown") {
				...
				const timer = setInterval(async () => {
					...
					let up = false;
					try {
						up = (await fetch("/api/dsh-power-button/health", { cache: "no-store" })).ok;
					} catch {
						up = false;
					}
					if (!up) {
			...
			(async () => {
				...
				if (!active()) return;
				fetch("/api/dsh-power-button/restart", {
					method: "POST",
					keepalive: true
				}).then(async (r) => {
					const j = await r.json().catch(() => ({}));
